...
### 4.3 引导程序完全同态
假设e是自举的。特别是,假设e可以处理以下四个函数:解秘函数,表示为一个大小为多项式namuta的电路De以及De增强的加、减,或者相乘模2门。我们将展示,这是一套完整的电路,如果这四个电路是在在允许函数集内,然后一旦可以从方案e中建立，那么e'是完全同态的。

作为一个热身,假设密文c1在pk1下进行加密。还假设我们有一个加密的私钥:让sk1作为一个密文的向量,它是在pk2下加密了sk1的。考虑以下的算法。

>公式

解密电路De已经有输入电线作为密钥和密文的比特。以上,Evaluatee采取sk1和c1,每个都在pk2下加密。然后,e被用于评估解密电路的同态性。只要e可以处理De,输出c就是一个pk2下加密的密文Decrypte(sk1 c1)=m。Recrypte因此输出一个新的pk2下的加密密文m。

Recrypte一个迷人之处是消息m是双重加密,首先在pk1下加密然后pk2下加密。通常,唯一一个可以研究双重加密消息的办法是首先剥离外层加密,然后解密内层。然而,在Recrypte中,Evaluatee算法用于消除内层加密,就像爱丽丝打开盒子#i虽然它是在盒子#(i + 1)内部。

想象e是我们的第三节中介绍的有点同态加密方案,并考虑一下Recrypte对暗文的噪音影响。评估De消除了噪声相关的第一个加密在pk1下的密文(当然,因为解密删除噪音),但Evaluatee同时引入了新的噪音。只要新的添加噪音小于旧的删除噪音,我们就是在不断进步。在爱丽丝的珠宝店类似的情况。当职工提取从手套箱#i中提取材料,同时这个过程消耗手套箱#(i + 1)。我们是“进步”的,只要这个过程没有离使得手套箱#(i + 1)完全坏掉。

当然,我们的目标是对消息执行实际操作,不仅仅是为了获得一个新的加密的相同的消息。所以,假设e可以通过门处理增强的De——例如,加法;称之为增强电路DAdd。如果c1和c2两个加密m1和m2的暗文,分别是加密在pk1下,那么我们计算c1_prime和c2_prime,作为pk2下的暗文加密,然后我们有

>公式

是一个加密在pk2下的m1异或m2。

通过递归这个过程,我们得到一个完全同态加密方案。e'内的公钥包含了一个公钥序列和一个加密的私钥链,其中ski是加密在pk(i+1)。为了评估e'中的函数f,我们将f表达为电路、把电路的门拓扑分级,然后顺序通过每一级。对于级i+1的门(如加门),我们采取输入作为加密的密钥ski_prime和一组输出相关的密文,我们同态评估DAdd得到在pki+1下加密的密文,级是i+1。最后,我们输出与f输出电路相关的密文。

把加密密钥比特sk1,……,skl放入方案e'的公钥内不是安全问题。只要e是语义安全,这些加密的秘钥和0的加密并没有什么区别。

### 4.4 电路安全

严格地说, E’不满足的定义完全同态加密,因为KeyGenE’线性增长的复杂性与最大电路深度我们要评估。(幸运的是,EncryptE’和DecryptE’不完全依赖于函数f的评价)。
然而,假设E不仅是引导程序,而且是电路安全的—也就是说,它是可以“安全”地揭秘加密密钥ski在自己的相关联pki公钥下。然后,我们可以简化KeyGenE’。我们不需要不同的公钥pki为每个电路水平和一个非循环链的加密密钥。相反,E’的公钥可以仅仅由一个公钥pk和一个加密密钥sk,在pk与各级电路相关联。这种方法有更多的优势,我们不需要事先决定我们希望能够评估的大电路深度函数的复杂性。

对于大多数加密方案,包括我们有点同态方案(据我们所知),揭示一个加密的sk pk不会导致任何攻击。然而,它通常很难证明一个加密方案是电路安全的。
电路安全的问题也适合我们的物理类比。假设一个钥匙是被锁在一个非常相似的箱子里,可以从外面用钥匙打开。会不会有可能使用手套和钥匙从内部打开这个箱子呢?如果是这样的话,这将是一个奇怪的锁。同样,加密方案在此设置往往是不安全的。

## 5. 总结

我们现在知道FHE是可能的。这里我们已经介绍了现有的方案,Gentry提供的基于格的方案由[2，3],和最近的一项Smart和Vercauteren的方案[7]。

要让FHE真正实用,还有工作要做。目前,所有已知FHE方案遵循上面的蓝图:构造一个可自举的有点同态加密方案,并获得通过对E的解密函数运行EvaluateE 来获得FHE。但这种方法需要大量的计算。不仅是将解密函数表达为(有点效率低下)电路,但随后EvaluateE会替换在这个电路中的每一位为一个很大的密文加密。也许有人会找到一个更有效的蓝图。

这里给出的方案,在概念上更简单,似乎不如基于格的方案有效。想要得到2λ安全对已知攻击——如。,在近似gcd的问题——暗文是λ5·polylog(λ)位,导致复杂度λ10·polylog(λ)的计算来评价解密功能。基于格的方案与相应的安全是复杂度为λ6·polylog(λ)的计算。这是很高,但并不是完全不合理。考虑:RSA 拥有2λ安全对应已知攻击,特别是与数域筛分解算法,您需要使用一个RSA模大约λ3比特。然后,RSA解密λ3比特指数——即涉及到求幂,例如关于λ3乘法。即使使用快速傅里叶乘法,这个取幂也需要复杂度λ6·polylog(λ)计算。也不像RSA解密函数在我们的方案是高度并行的,并行可能会在某些实践中产生一个巨大的差异。

## 6. 后记
她做那个梦后的第二天早上,爱丽丝把她的手套箱解决方案解释给她的工人们听。他们并不高兴,但是他们希望被继续雇用。随着时间的推移,很明显,手套箱使得珠宝的组装速度大大减慢了。主要的问题似乎是厚手套,使得每个组装步骤所需的时间都增长了好几倍。低产出的几天后,爱丽丝开始限制使用手套箱，仅仅使用在最有价值的钻石上。

爱丽丝败诉了，在诉讼制作手套箱的Acme公司中,因为,所有人都知道在爱丽丝的平行世界,手套,手套箱总是很僵硬,适度使用后会完全变硬。老法官对她用傲慢的语气解释道。

但爱丽丝拒绝放弃。她雇佣了一个年轻英俊的杂物箱研究员，让他开发一个足够灵活的手套,允许灵活地组装珠宝和打开箱子,但足够坚固,可以防止箱子被破解开。这个研究员,惊讶于他的好运气,陷入了这个问题之中。
